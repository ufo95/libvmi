...
status_t
is_WINDOWS_KERNEL(vmi_instance_t vmi, addr_t base_v, uint8_t * pe)
{
    ...
    uint32_t nbytes =
        vmi_read_va(vmi, base_v + export_header_offset, 0, &et,
...
status_t
check_os_version(vmi_instance_t vmi, addr_t kernel_base_v, uint8_t * pe)
{
...
status_t
check_guid(vmi_instance_t vmi, addr_t kernel_base_v, uint8_t * pe)
{
    ...
    vmi_read_va(vmi, kernel_base_v + debug_offset, 0,
    ...
    vmi_read_va(vmi, kernel_base_v + debug_directory.address_of_raw_data, 0,
...
status_t
check_pe_sections(vmi_instance_t vmi, addr_t image_base_v, uint8_t * pe)
{
...
START_TEST(test_peparse)
{
    ...
    if (VMI_OS_WINDOWS == vmi_get_ostype(vmi)) {
        ...
        if (VMI_SUCCESS ==
            peparse_get_image_virt(vmi, kernbase, 0, MAX_HEADER_SIZE, pe)) {
