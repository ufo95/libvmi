...
char *
rva_to_string(vmi_instance_t vmi, addr_t rva, addr_t base_addr, uint32_t pid)
{
...
get_aof_index(vmi_instance_t vmi, int aon_index, struct export_table *et, addr_t base_addr, uint32_t pid)
{
    ...
    if (vmi_read_16_va(vmi, aof_index_loc, pid, &aof_index) == VMI_SUCCESS) {
...
windows_export_to_rva(vmi_instance_t vmi, char *symbol, addr_t base_vaddr, uint32_t pid, addr_t *rva)
{
    ...
    if (peparse_get_export_table(vmi, base_vaddr, pid, &et, &et_rva, &et_size)
        != VMI_SUCCESS) {
