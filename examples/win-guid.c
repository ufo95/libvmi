...
status_t
is_WINDOWS_KERNEL(vmi_instance_t vmi, addr_t base_p, uint8_t * pe)
{
...
void
print_os_version(vmi_instance_t vmi, addr_t kernel_base_p, uint8_t * pe)
{
...
void
print_guid(vmi_instance_t vmi, addr_t kernel_base_p, uint8_t * pe)
{
    ...
    int c;
    for (c = 0; c < 8; c++)
        printf("%.2x", pdb_header->signature.data4[c]);
    ...
    } else if (!strcmp("ntkrnlmp.pdb", pdb_header->pdb_file_name)) {
        printf("\tMulti-processor without PAE\n");
    } else if (!strcmp("ntkrnlpa.pdb", pdb_header->pdb_file_name)) {
        printf("\tSingle-processor with PAE (version 5.0 and higher)\n");
    } else if (!strcmp("ntkrpamp.pdb", pdb_header->pdb_file_name)) {
...
void
print_pe_header(vmi_instance_t vmi, addr_t image_base_p, uint8_t * pe)
{
